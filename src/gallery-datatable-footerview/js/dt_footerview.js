/**
 FooterView is a YUI View class extension that provides a simple, one row
 summary row to a Datatable (either for scrolling or non-scrolling DataTable).

 This view provides for a single summary row, typically with one TH element (with a colspan)
 and several TD elements for each column where a "calculated field" is desired.

 View configuration provides for calculated fields based upon the currently displayed
 dataset or fields.
 ...
 * fixed
 ...
 { key:'pdbl', calc:"{sum}", className:"align-right" }
 ]
 }
 });
...
Y.FooterView = Y.Base.create( 'tableFooter', Y.View, [], {
    ...
    initializer: function(config) {
        ...
        if ( config.footerConfig && config.footerConfig.fixed && this._dt.get('scrollable') ) {
            //   if ( this._chkVersion() === 0 ) {
            //       this._subscr.push( Y.Do.after( this._buildFixedFooter, this._dt, '_syncScrollUI', this._dt) );
            //       this._subscr.push( Y.Do.after( this.resizeFooter, this._dt, '_syncScrollUI', this) );
            //   } else {
            this._subscr.push( Y.Do.after( this._buildFixedFooter, this._dt, '_syncScrollUI', this._dt) );
            //   }
        }

        this._subscr.push( this.get('modelList').after(['*:change','*:add','*:create', '*:remove', '*:reset'], Y.bind('refreshFooter', this) ) );
...
// --------------------------------------------------------------------------------
     ...
     *
     *  NOTE: This view's render method is called prior to data existing
     *        In the current version (3.6.0pr4), the TFOOT for a scrolling DT is created
     *        in an incorrect location, thus the method _buildFixedFooter is called.
     *
     * @method renderFooter
     * @public
     * @chainable
    ...
    renderFooter: function(){
        ...
        Y.Array.each( foot_cols, function(fitem){
            ...
            if ( imatch !== -1) {
                ...
                td_html[imatch].calc = fitem.calc || null;
        ...
        var tr_tmpl = this.TMPL_tfoot;
        //   if ( this._chkVersion() === 1 ) {
        tr_tmpl = this.TMPL_tfoot;
        //   } else {
        //       tr_tmpl = this.TMPL_tr;
        //   }
    ...
    refreshFooter: function(){
        ...
        Y.Array.each( this.node_cols, function(fitem,findex) {
            ...
            if ( fitem.th ) {
                ...
                td_html = this.fnReplace( fitem.th.label, replacer_obj );
            }

            if ( !fitem.th && fitem.calc ) {
    ...
    resizeFooter : function() {
        var table_obj = this._dt,
            ...
            tfoot_nodes = tfootNode.all('th,td'), //this._dt._yScrollFooter.all('th,td'),
        //tfoot_nodes = this.get('container').all('th,td'),
            td_first = true;

        if( table_obj._yScroll ) {//table_obj.get('scrollable') ) {

            var thead_ths = thead.all('th'),
                num_cols  = this.node_cols.length;
            ...
            Y.Array.each( this.node_cols, function(col,i) {
                ...
                if ( col.th ) {
                    for(var j=0; j<col.th.colspan; j++) {
                        ...
                        col_width += +(thead_th.getComputedStyle('width').replace(/px/,''));
                        //    col_width += parseFloat(thead_th.getComputedStyle('width').replace(/px/,''));
                    }
                    col_width += col.th.colspan-1;
                } else {
                    thead_th = thead_ths.item(col.index);
                    col_width = +(thead_th.getComputedStyle('width').replace(/px/,''))-20;
                    //col_width = parseFloat(thead_th.getComputedStyle('width'))-20;
                }
