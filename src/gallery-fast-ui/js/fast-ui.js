function getNode(rootNode, id) {
    return rootNode.get("id") === id ? rootNode : rootNode.one("#" + id);
}
...
function getClass(fullName) {
    if(/^Y\./.test(fullName)) {
        var m = /^Y\.((.*)\.)?(.*?)$/.exec(fullName), packageName = m[2], className = m[3];
        if(packageName) {
...
function createWidget(rootNode, customWidget, config) {
    var Clazz = getClass(customWidget.fullName), result;
...
Y.fastUi = function (parent, xmlContent, msg, config) {
    var guiCreationContext = preXmlParse(xmlContent), widgets = guiCreationContext.widgets, htmlCode = guiCreationContext.html, variables = guiCreationContext.variables, translatedHtml = msg ? Y.substitute(htmlCode, msg) : htmlCode, closedNodeHtmlBugFix = translatedHtml.replace(/<([\w\d]+?)\s+([^>]+?)\/>/gm, "<$1 $2></$1>"), rootNode = [
        Y.Node.create(closedNodeHtmlBugFix)
    ], createdWidgets = {
    }, result, i, variable, node, newWidget;
    if(parent) {
        parent.appendChild(rootNode[0]);
    }
    result = {
...
//@ sourceMappingURL=fast-ui.js.map
