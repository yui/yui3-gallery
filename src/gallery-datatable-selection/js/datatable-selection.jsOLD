...
//------------------------------------------------------------------------------------------------------
    ...
    _selectedChange: function(o){
        ...
        if(this.get('selectionMode').toLowerCase && this.get('selectionMode')==='cell') {
            sobj.cells = this.get('selectedCells');
        } else if (this.get('selectionMode').toLowerCase && this.get('selectionMode')==='row') {
    ...
    _setSelectedCells: function(val){
        ...
        if ( Y.Lang.isArray(val) && this.data.size() > val.length ) {
            Y.Array.each(val,function(item) {
                var row, col, td, ckey;
                row = ( Y.Lang.isNumber(item.record) ) ? this.getRow( item.record ) : row;
